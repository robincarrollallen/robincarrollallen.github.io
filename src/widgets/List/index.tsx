...
import { LOAD_MORE_STATUS, type LoadMoreType } from '~/enums/status'
import { YStack, isWeb, Spinner, Text, useTheme, type TamaguiElement } from 'tamagui'
import { forwardRef, memo, useCallback, useImperativeHandle, useRef, useState } from 'react'
import BigList, { type BigListProps } from 'react-native-big-list'
...
export const List = forwardRef<BigList<any>, ListProps<any>>(({
  ...
  const ListComponent = (
    <BigList
      ...
      keyExtractor={(item: Recordable) => item.id}
  ...
  const EmptyComponent = memo(() => (
    <YStack flex={1} items="center" pb={footerHeight as number || 0} pt="25%">
      ...
      <Text color={theme.textWeakest?.val}>{emptyText || 'No Record'}</Text>
    </YStack>
  ))
...
const WebPullToRefresh = forwardRef<WebPullToRefreshRef, WebPullToRefreshProps>(({ children, onRefresh, refreshing, scrollOffset }, ref) => {
  ...
  const containerRef = useRef<TamaguiElement>(null) // 下拉容器引用
  const startY = useRef(0) // 开始触摸的Y坐标

  const handleTouchStart = (event: Recordable) => {
  ...
  const handleTouchMove = (event: Recordable) => {
        ...
        >
          ...
          >
            { 
              ...
              : <SvgXml xml={SVG.loading} width={42} height={42} transform={[{ rotate: `${pullDistance * 2}deg` }]} />
            }
          </YStack>
        </YStack>
      )}
