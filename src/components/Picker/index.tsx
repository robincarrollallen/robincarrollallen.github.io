import { Popover, YStack, Text, ScrollView, type PopoverProps, useTheme } from "tamagui"
import { Pressable, useWindowDimensions } from "react-native"
import { useSizeTokens } from "~/store/modules/responsive"
import { POPOVER_PLACEMENT } from "~/constants/layout"
import { memo, useRef, useState } from "react"

/** Picker Component Props Type */
...
export const Picker = memo(({
  ...
  return (
    <Popover open={open} placement={placement} onOpenChange={handleOpenChange}>
      ...
      >
        {/* 箭头(默认高度0) */}
        <Popover.Arrow borderWidth={1} borderColor={theme.borderColor?.val} />
        {
          renderContent
          ? renderContent(close)
              ...
              <YStack gap={10}>
                {items.map((item) => (
                  <Text
                    ...
                    color={value === item.value ? theme.color?.val : theme.textWeaker?.val}
                    bg={value === item.value ? theme.surfaceRaisedL2?.get() : null}
                  >
                    {item.label}
                  </Text>
                ))}
